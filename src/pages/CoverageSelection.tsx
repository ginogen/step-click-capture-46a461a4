...
const CoverageSelection = () => {
  ...
  return <div className="min-h-screen flex flex-col items-center justify-start p-4 bg-white">
      <div className="w-full max-w-md mx-auto pt-8 pb-6">
        <div className="flex justify-center mb-8">
          <img alt="Grupo Cazalá Seguros" className="h-16 object-contain" src="/lovable-uploads/21f48b1c-97c2-4940-b039-01abe41b8de9.png" />
        </div>
        ...
        {!selectedCategory ? <div className="grid grid-cols-1 gap-4 w-full max-w-lg">
            <Button onClick={() => handleCategorySelect("auto")} className="py-6 bg-white hover:bg-gray-50 text-black border border-gray-200 shadow-sm transition-all duration-200 rounded-xl" variant="outline">
            ...
            <Button onClick={() => setSelectedCategory("other")} className="py-6 bg-white hover:bg-gray-50 text-black border border-gray-200 shadow-sm transition-all duration-200 rounded-xl" variant="outline">
              <div className="flex flex-col items-center">
            ...
            <div className="grid grid-cols-1 gap-4 w-full max-w-xl">
              {AUTO_COVERAGE_TYPES.map(coverage => <Button key={coverage.id} onClick={() => handleCoverageSelect(coverage.id)} className="py-5 bg-white hover:bg-gray-50 text-black border border-gray-200 shadow-sm transition-all duration-200 rounded-xl" variant="outline">
                  <div className="flex flex-col items-center">
            ...
            <Button onClick={() => setSelectedCategory(null)} className="mt-8 text-gray-600 mx-auto block" variant="ghost">
              Volver
            </Button>
          </> : <>
            <h3 className="text-2xl font-semibold mb-6 text-center">Coberturas Varias</h3>
            <div className="grid grid-cols-1 gap-4 w-full max-w-xl">
              {OTHER_COVERAGE_TYPES.map(coverage => <Button key={coverage.id} onClick={() => handleCoverageSelect(coverage.id)} className="py-5 bg-white hover:bg-gray-50 text-black border border-gray-200 shadow-sm transition-all duration-200 rounded-xl" variant="outline">
                  <div className="flex flex-col items-center">
                    <span className="font-semibold text-lg">{coverage.name}</span>
                    <span className="text-sm text-gray-500 mt-1">{coverage.requiredPhotos} fotos</span>
                  </div>
      ...
      <Dialog open={showAutoModal} onOpenChange={setShowAutoModal}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button onClick={continueToAutoCoberages} className="w-full">
              Entendido, continuar
            </Button>
          </DialogFooter>
        </DialogContent>
